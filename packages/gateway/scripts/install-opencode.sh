...
echo "[chatcode] Installing OpenCode CLI for the Chatcode gateway host..."

if [ "${EUID:-$(id -u)}" -eq 0 ]; then
    echo "[chatcode] ERROR: run this installer as the target non-root user so OpenCode is installed under that user's home" >&2
    exit 1
fi
...
OPENCODE_GUIDANCE_FILE="${OPENCODE_CONFIG_DIR}/AGENTS.md"
...
ensure_node() {
    ...
    if [ "${os}" = "Darwin" ]; then
        if ! command -v brew &>/dev/null; then
            cat >&2 <<'EOF'
[chatcode] ERROR: Homebrew is required on macOS to install Node.js 24 for OpenCode.
[chatcode] Run:
...
seed_default_opencode_guidance() {
    install -d -m 755 "${OPENCODE_CONFIG_DIR}"
    if [ -e "${OPENCODE_GUIDANCE_FILE}" ]; then
        echo "[chatcode] OpenCode global guidance already exists at ${OPENCODE_GUIDANCE_FILE}; leaving it untouched"
        return 0
    fi

    cat > "${OPENCODE_GUIDANCE_FILE}" <<'EOF'
...
# Install OpenCode CLI in the target user's local prefix.
npm_user_install "opencode-ai@latest"
seed_default_opencode_guidance

# Verify installation
if ! command -v opencode &>/dev/null; then
    echo "[chatcode] ERROR: opencode not found in PATH after install" >&2
    exit 1
fi

echo "[chatcode] OpenCode installed: $(opencode --version)"
