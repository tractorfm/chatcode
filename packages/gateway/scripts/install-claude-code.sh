#!/usr/bin/env bash
# Install Claude Code CLI on the gateway host for the target user.
...
echo "[chatcode] Installing Claude Code for the Chatcode gateway host..."

if [ "${EUID:-$(id -u)}" -eq 0 ]; then
    echo "[chatcode] ERROR: run this installer as the target non-root user so Claude Code is installed under that user's home" >&2
    exit 1
fi
...
CLAUDE_GUIDANCE_FILE="${CLAUDE_DIR}/CLAUDE.md"
...
ensure_node() {
    ...
    if [ "${os}" = "Darwin" ]; then
        if ! command -v brew &>/dev/null; then
            cat >&2 <<'EOF'
[chatcode] ERROR: Homebrew is required on macOS to install Node.js 24 for Claude Code.
[chatcode] Run:
...
seed_default_claude_guidance() {
    install -d -m 755 "${CLAUDE_DIR}"
    if [ -e "${CLAUDE_GUIDANCE_FILE}" ]; then
        echo "[chatcode] Claude global guidance already exists at ${CLAUDE_GUIDANCE_FILE}; leaving it untouched"
        return 0
    fi

    cat > "${CLAUDE_GUIDANCE_FILE}" <<'EOF'
# Chatcode Global CLAUDE.md
...
# Install/upgrade Claude Code in the target user's local prefix.
npm_user_install "@anthropic-ai/claude-code@latest"
seed_default_claude_guidance

# Verify installation
if ! command -v claude &>/dev/null; then
    echo "[chatcode] ERROR: claude not found in PATH after install" >&2
    exit 1
fi

echo "[chatcode] Claude Code installed: $(claude --version)"
