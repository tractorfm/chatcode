# chatcode.dev – Implementation Plan: Milestone 2
...
- ✅ Route/DO tests for `GatewayHub`, `auth`, `vps`, `sessions`
- ✅ Control-plane typecheck + tests pass
- ✅ Wrangler pinned to v4 (`^4.68.1`)
...
- ✅ Snapshot route now validates that the requested session belongs to the VPS
- ✅ Event forwarding typing fixed for strict TypeScript build

### Remaining M2 validation work
- Staging deploy + remote migration verification
- End-to-end smoke run against real DO resources
...
└── test/
    ...
    ├── gateway-hub.test.ts       # GatewayHub pending/snapshot/gateway-id behavior
    ├── routes.auth.test.ts       # OAuth route behavior
    ├── routes.vps.test.ts        # VPS delete/reconciliation route behavior
    ├── routes.sessions.test.ts   # session create/snapshot route behavior
    └── tsconfig.json
```
...
12. **Tests** – utility/unit + route/DO suites landed (`auth`, `do-tokens`, `ids`, `GatewayHub`, `routes.auth`, `routes.vps`, `routes.sessions`); extend with deeper reconnect/file-transfer cases as needed
...
- **Current automated coverage**: auth token/cookie sign/verify, AES-GCM encrypt/decrypt round-trip, ID/random generation utilities, GatewayHub command/snapshot/gateway-id behavior, route tests (`auth`, `vps`, `sessions`), control-plane typecheck/build, local D1 migration/foreign-key checks.
